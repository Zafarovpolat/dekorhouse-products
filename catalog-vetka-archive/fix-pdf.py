...
SRC = os.path.join(os.path.dirname(os.path.abspath(__file__)), "catalog-vetka-archive.pdf")
...
if __name__ == '__main__':
    ...
    if which in ('both', 'printer'):
        build(base + "catalog-vetka-archive-printer.pdf", label="ПЕЧАТНАЯ (полное качество)")
    if which in ('both', 'ebook'):
        build(base + "catalog-vetka-archive-ebook.pdf", maxside=1000, quality=72,
